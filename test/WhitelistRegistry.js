    ...
    describe('register', async () => {
        it('should whitelist 10 addresses', async () => {
            ...
            for (let i = 1; i <= MAX_WHITELSITED; ++i) {
                ...
                expect(await this.WhitelistRegistry.status(addrs[i])).to.be.bignumber.equal(toBN('1'));
            }
        });

        it('should fail to whitelist due to balance lower than threshold', async () => {
            await expect(this.WhitelistRegistry.register({ from: addrs[1] }))
                .to.eventually.be.rejectedWith('BalanceLessThanThreshold()');
        });
        ...
        it('should whitelist 10 addresses, then fail to whitelist due to not enough balance, then whitelist successfully', async () => {
            ...
            await expect(this.WhitelistRegistry.register({ from: addrs[MAX_WHITELSITED + 1] })).to.eventually.be.rejectedWith('NotEnoughBalance()');
            expect(await this.WhitelistRegistry.status(addrs[MAX_WHITELSITED + 1])).to.be.bignumber.equal(toBN('0'));
            for (let i = 1; i <= MAX_WHITELSITED; ++i) {
                await this.Staking.transfer(addrs[0], THRESHOLD, { from: addrs[i] });
                expect(await this.WhitelistRegistry.status(addrs[i])).to.be.bignumber.equal(toBN('1'));
            }
            await this.Staking.transfer(addrs[MAX_WHITELSITED + 1], THRESHOLD);
            await this.WhitelistRegistry.register({ from: addrs[MAX_WHITELSITED + 1] });
            expect(await this.WhitelistRegistry.status(addrs[MAX_WHITELSITED + 1])).to.be.bignumber.equal(toBN('1'));
            expect(await this.WhitelistRegistry.status(addrs[1])).to.be.bignumber.equal(toBN('0'));
        ...
        it('should whitelist 10 addresses, then lower balance, then whitelist successfully', async () => {
            ...
            await this.Staking.transfer(addrs[0], toBN('1'), { from: addrs[3] });
            expect(await this.WhitelistRegistry.status(addrs[3])).to.be.bignumber.equal(toBN('1'));
            await this.WhitelistRegistry.register({ from: addrs[MAX_WHITELSITED + 1] });
            expect(await this.WhitelistRegistry.status(addrs[MAX_WHITELSITED + 1])).to.be.bignumber.equal(toBN('1'));
            expect(await this.WhitelistRegistry.status(addrs[3])).to.be.bignumber.equal(toBN('0'));
        });
        ...
        it('should whitelist 10 addresses, then whitelist 9 times successfully', async () => {
            ...
            let whitelisted = 0;
            for (let i = 1; i <= MAX_WHITELSITED; ++i) {
                whitelisted += (await this.WhitelistRegistry.status(addrs[i])).toNumber();
            }
