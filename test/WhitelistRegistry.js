...
describe('WhitelistRegistry', function () {
    ...
    describe('register', function () {
        ...
        it('should fail to whitelist due to percentage lower than threshold', async function () {
            const { rewardableDelegationPod, whitelistRegistry } = await loadFixture(initContracts);
        ...
        it('should fail to whitelist the same address twice', async function () {
            const { rewardableDelegationPod, whitelistRegistry } = await loadFixture(initContracts);
        ...
        it('should whitelist 10/10 addresses, then fail to whitelist due to not enough balance, then move power and whitelist successfully', async function () {
            const { rewardableDelegationPod, whitelistRegistry } = await loadFixture(initContracts);
            for (let i = 1; i <= WHITELIST_LIMIT; ++i) {
                await rewardableDelegationPod.mint(addrs[i].address, RESOLVER_BALANCE);
            }
        ...
        it('should whitelist 5/10 addresses, then lower balance, then whitelist +1 successfully', async function () {
            const { rewardableDelegationPod, whitelistRegistry } = await loadFixture(initContracts);
        ...
        it('should whitelist 5/10 addresses, then lower balance, then whitelist instead successfully', async function () {
            const { rewardableDelegationPod, whitelistRegistry } = await loadFixture(initContracts);
            const resolversNumber = WHITELIST_LIMIT / 2;
            for (let i = 1; i <= resolversNumber; ++i) {
                await rewardableDelegationPod.mint(addrs[i].address, RESOLVER_BALANCE);
    ...
    describe('setResolverPercentageThreshold', function () {
        ...
        describe('decrease less than current resolver percentage threshold', function () {
            ...
            it('should allow to add addresses with suitable balance', async function () {
                ...
                console.log('resolversToRegister', resolversToRegister);
