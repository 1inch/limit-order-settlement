...
const { buildOrder, signOrder, buildTakerTraits, fillWithMakingAmount, buildMakerTraits } = require('@1inch/limit-order-protocol-contract/test/helpers/orderUtils');
...
describe('MeasureGas', function () {
    ...
    async function initContracts() {
        ...
        return {
            contracts: { dai, weth, swap, settlement, settlementExtension, feeBank, resolvers },
    ...
    it('1 fill for 1 order', async function () {
        const { contracts: { dai, weth, swap, settlement, resolvers }, accounts: { owner, alice }, others: { chainId } } = await loadFixture(initContractsAndApproves);
        ...
        const { r, _vs: vs } = ethers.utils.splitSignature(await signOrder(order, chainId, swap.address, alice));

        const fillOrderToData = swap.interface.encodeFunctionData('fillOrderArgs', [
            order,
            r,
            vs,
            ether('100'),
    ...
    it.only('extension 1 fill for 1 order', async function () {
        const { contracts: { dai, weth, swap, settlementExtension }, accounts: { owner, alice }, others: { chainId } } = await loadFixture(initContractsAndApproves);
        ...
        const tx = await swap.fillOrderArgs(
            order,
            r,
            vs,
            ether('100'),
