...
describe('Settlement', async () => {
    ...
    describe('WhitelistChecker', async () => {
        describe('should not work with non-whitelisted address', async () => {
            ...
            afterEach(async () => {
                if (!this.skipAfterEach) {
                    await this.whitelistRegistrySimple.setStatus(addr0, Status.Verified);
                    this.skipAfterEach = false;
                }
            });
            ...
            it('onlyWhitelisted modifier in matchOrders method with LimitOrderProtocol via interaction and FailedExternalCall', async () => {
                ...
                const interaction = this.matcher.address + '01' + web3.eth.abi.encodeParameters(
                    ...
                    [
                        ...
                        [this.matcher.contract.methods.matchOrders(this.swap.address, order, '0x', '0x', ether('0.01'), 0, ether('10')).encodeABI()],
                    ],
                ).substring(2);
                await expect(this.swap.fillOrder(order, signature, interaction, ether('10'), 0, ether('0.01')))
                    .to.eventually.be.rejectedWith('FailedExternalCall()');
            });

            it('onlyWhitelisted modifier in matchOrders method with LimitOrderProtocol', async () => {
                const SettlementMock = artifacts.require('SettlementMock');
                const matcherMock = await SettlementMock.new(this.whitelistRegistrySimple.address, this.swap.address);
                ...
                const preInteraction = matcherMock.address + '00' + matcherMock.contract.methods.matchOrders(
                    this.swap.address,
                    order0,
                    '0x',
                    '0x',
                ...
                const signature = signOrder(order, this.chainId, this.swap.address, addr1Wallet.getPrivateKey());
                await expect(this.swap.fillOrder(order, signature, '0x', ether('10'), 0, ether('0.01')))
                    .to.eventually.be.rejectedWith('AccessDenied()');
            });
        ...
        describe('should work with whitelisted address', async () => {
            const tryMatchOrder = async (matchOrderMethod) => {
                const order0 = buildOrder({ makerAsset: this.dai.address, takerAsset: this.weth.address, makingAmount: ether('100'), takingAmount: ether('0.1'), from: addr0 });
            ...
            it('onlyWhitelistedEOA modifier in matchOrdersEOA method', async () => {
                ...
                await tryMatchOrder(this.matcher.matchOrdersEOA);
            ...
            it('onlyWhitelisted modifier in matchOrders method', async () => {
                ...
                await tryMatchOrder(this.matcher.matchOrders);
